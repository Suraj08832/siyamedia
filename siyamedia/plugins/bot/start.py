...
from siyamedia.misc import _boot_
from siyamedia.plugins.sudo.sudoers import sudoers_list
from siyamedia.utils import bot_sys_stats
...
from siyamedia.utils.decorators.language import LanguageStart
from siyamedia.utils.formatters import get_readable_time
from siyamedia.utils.inline.start import private_panel, start_panel
from siyamedia.utils.inline.help import first_page
...
async def start_pm(client, message: Message, _):
        ...
        if name.startswith("sud"):
            ...
            if await is_on_off(2):
                ...
                await app.send_message(
                    ...
                    text=(
                        f"{message.from_user.mention} ??s? s?????? ??? ??? ?? ????? <b>s?????s?</b>.\n\n"
                        f"<b>?s?? ?? :</b> <code>{message.from_user.id}</code>\n"
                        f"<b>?s?????? :</b> {username}"
                    ),
                )
            return
            ...
            try:
                ...
                if await is_on_off(2):
                    ...
                    await app.send_message(
                        chat_id=config.LOGGER_ID,
                        text=(
                            f"{message.from_user.mention} ??s? s?????? ??? ??? ?? ????? <b>????? ???????????</b>.\n\n"
                            f"<b>?s?? ?? :</b> <code>{message.from_user.id}</code>\n"
                            f"<b>?s?????? :</b> {username}"
                        ),
                    )
    ...
    if await is_on_off(2):
        ...
        await app.send_message(
            chat_id=config.LOGGER_ID,
            text=(
                f"{message.from_user.mention} ??s? s?????? ??? ???.\n\n"
                f"<b>?s?? ?? :</b> <code>{message.from_user.id}</code>\n"
                f"<b>?s?????? :</b> {username}"
            ),
        )
