...
def _view_btn(msg: Message):
    try:
        return InlineKeyboardMarkup([[InlineKeyboardButton("?? ???? ??ss???", url=msg.link)]])
    except Exception:
        return None
...
async def set_photo(_, message: Message):
    ...
    if not file_id:
        return await message.reply_text("**????s? ????? ???? ?? ????? (????? ?? ????? ????????).**")

    try:
        await app.set_chat_photo(chat_id=message.chat.id, photo=file_id)
        await message.reply_text(f"**????? ????? ??????? s????ss?????!**\n?? {message.from_user.mention}")
    except Exception as e:
...
async def set_description(_, message: Message):
    ...
    try:
        ...
        await message.reply_text(f"**?????? ?? s?? ??s????????:**\n`{e}`")
