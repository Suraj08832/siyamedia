...
from siyamedia.utils.admin_check import is_admin
from siyamedia.plugins.misc.funtag_messages import (
    GN_MESSAGES,
    GM_MESSAGES,
    HI_MESSAGES,
    QUOTES,
...
async def mention_members(client, message, message_pool, stop_cmd):
    ...
    if message.chat.type == ChatType.PRIVATE:
        return await message.reply_text("? ???s ??????? ????s ???? ?? ?????s.")
    ...
    try:
        ...
        await client.send_message(chat_id, "? ??????? s?ss??? ?????.")
...
async def stop_tagging(client, message):
    ...
    if not await is_admin(message):
        return await message.reply_text("?? ???? ?????s ??? s??? ???????.")

    if chat_id not in spam_chats:
        return await message.reply_text("?? ?? ?????? ??????? s?ss??? ?????.")

    spam_chats.discard(chat_id)
    active_tags.pop(chat_id, None)
    await message.reply_text("? ?????????? s?????? s????ss?????.")
