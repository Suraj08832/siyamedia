...
from siyamedia.mongo.welcomedb import is_on, set_state, bump, cool, auto_on

BG_PATH = "siyamedia/assets/annie/welcome.png"
FALLBACK_PIC = "siyamedia/assets/upic.png"
FONT_PATH = "siyamedia/assets/annie/Arimo.ttf"
...
async def toggle(client, m: Message):
    ...
    if u.status not in (enums.ChatMemberStatus.ADMINISTRATOR, enums.ChatMemberStatus.OWNER):
        return await m.reply_text("**s???? ???? ?????s ??? ?????? ??????? ???????????? s????s!**")
    flag = m.command[1].lower()
    if flag not in ("on", "off"):
        return await m.reply_text("**Usage:**\n?/welcome [on|off]\n? Annie Special Welcome.....")
    cur = await is_on(m.chat.id)
    if flag == "off" and not cur:
        return await m.reply_text("**??????? ???????????? ??????? ??s?????!**")
    if flag == "on" and cur:
        return await m.reply_text("**??????? ???????????? ??????? ???????!**")
    await set_state(m.chat.id, flag)
    await m.reply_text(f"**{'???????' if flag == 'on' else '??s?????'} ??????? ?? {m.chat.title}**")
...
async def welcome(client, update: ChatMemberUpdated):
    ...
    if not await is_on(cid):
        if await auto_on(cid):
            await safe_send(client.send_message, cid, "**??????? ??ss???s ??-???????.**")
        else:
            return
    ...
    if burst >= JOIN_THRESHOLD:
        ...
        await safe_send(client.send_message, cid, f"**??ss??? ???? ???????? (x{burst}). ??????? ??ss???s ??s????? ??? {minutes} ??????s.**")
        return
    ...
    async def cleanup(path):
        if path and os.path.exists(path) and not os.path.abspath(path).startswith(os.path.abspath("siyamedia/assets")):
            try:
                os.remove(path)
            except:
                pass
