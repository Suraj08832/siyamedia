...
from siyamedia.misc import HAPP, SUDOERS, XCB
...
async def update_(client, message, _):
    ...
    for info in repo.iter_commits(f"HEAD..origin/{config.UPSTREAM_BRANCH}"):
        updates += (
            f"<b>\u2793 #{info.count()}: <a href={REPO_}/commit/{info}>{info.summary}</a> ?? -> {info.author}</b>\n"
            f"\t\t\t\t<b>\u279e ???????? ?? :</b> {ordinal(int(datetime.fromtimestamp(info.committed_date).strftime('%d')))} "
    ...
    if await is_heroku():
        ...
        os.execv(sys.executable, [sys.executable, "-m", "siyamedia"])


@app.on_message(filters.command(["restart"]) & SUDOERS)
async def restart_(_, message):
    response = await message.reply_text("??s???????...")
    ac_chats = await get_active_chats()
    for x in ac_chats:
        try:
            await app.send_message(
                chat_id=int(x),
                text=f"{app.mention} ?s ??s???????...\n\n??? ??? s???? ??????? ????? ????? 15-20 s?????s.",
            )
            await remove_active_chat(x)
            await remove_active_video_chat(x)
        except:
