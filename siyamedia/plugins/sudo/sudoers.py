...
from siyamedia.utils.database import add_sudo, remove_sudo
from siyamedia.utils.decorators.language import language
from siyamedia.utils.extraction import extract_user
...
async def sudoers_list(client, message: Message):
    keyboard = [[InlineKeyboardButton("? ???? s?????s? ?", callback_data="sudo_list_view")]]
...
async def view_sudo_list_callback(client, callback_query: CallbackQuery):
    if callback_query.from_user.id not in SUDOERS:
        return await callback_query.answer("???? s?????s ??? ????? ??? ????ss ???s", show_alert=True)

    owner = await app.get_users(OWNER_ID)
    caption = f"**???s? ?? ??? ?????????s?**\n\n**??O????** ? {owner.mention}\n\n"
    keyboard = [[InlineKeyboardButton("? ???? ????? ?", url=f"tg://openmessage?user_id={OWNER_ID}")]]
    ...
    for user_id in SUDOERS:
        ...
        try:
            ...
            count += 1
            caption += f"**?? S??? {count} »** {user.mention}\n"
            keyboard.append([
                InlineKeyboardButton(f"? ???? s??? {count} ?", url=f"tg://openmessage?user_id={user_id}")
            ])
        except Exception:
            continue
    ...
    keyboard.append([InlineKeyboardButton("? ???? ?", callback_data="sudo_list_back")])
