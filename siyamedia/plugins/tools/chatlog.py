    ...
    try:
        ...
        for member in message.new_chat_members:
            ...
            caption = (
                ...
                f"?? **???? ????:** `{chat.title}`\n"
                f"?? **???? ??:** `{chat.id}`\n"
                f"?? **???? ?s??????:** @{chat.username if chat.username else 'Private'}\n"
                f"?? **???? ????:** [????? ????]({invite_link or 'https://t.me/'})\n"
                f"?? **????? ??????s:** `{member_count}`\n"
            ...
            if _is_valid_url(invite_link):
                reply_markup = InlineKeyboardMarkup(
                    [[InlineKeyboardButton("s?? ????? ??", url=invite_link.strip())]]
                )
...
async def on_left_chat_member(_, message: Message):
    try:
        ...
        remover = message.from_user.mention if message.from_user else "**??????? ?s??**"
        chat = message.chat
