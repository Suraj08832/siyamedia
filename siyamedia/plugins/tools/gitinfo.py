...
async def github(_, message: Message):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            return await message.reply_text(
                f"?? <b>??Q??s? ??????:</b> <code>{html.escape(str(e))}</code>",
                parse_mode=ParseMode.HTML,
                disable_web_page_preview=True,
            )
    ...
    profile_link = (
        f'<a href="{html.escape(profile_url)}">???? ?? ??????</a>' if profile_url else "N/A"
    )

    caption = (
        "<b>? ?????? ??????? ????</b>\n\n"
        f"?? <b>????:</b> <code>{name}</code>\n"
        f"?? <b>?s??????:</b> <code>{_safe(username)}</code>\n"
        f"?? <b>???:</b> {bio}\n"
        f"?? <b>???????:</b> {company}\n"
        f"?? <b>????????:</b> {location}\n"
        f"?? <b>????:</b> {blog_disp}\n"
        f"?? <b>??????? ??:</b> <code>{created}</code>\n"
        f"?? <b>?????? ????s:</b> <code>{repos}</code>\n"
        f"?? <b>????????s:</b> <code>{followers}</code> | "
        f"<b>?????????:</b> <code>{following}</code>\n"
        f"?? <b>???????:</b> {profile_link}"
    )
