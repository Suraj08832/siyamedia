...
from siyamedia.utils.files import resize_file_to_sticker_size
...
async def kang(client, message):
    notify = await message.reply_text("? ?????ss???…")
    ...
    try:
        ...
        kind_lbl = "????????" if is_anim else "?????" if is_vid else "s?????"

        await notify.edit(
            f"? {'???????' if created else '????? ??'} ???? {kind_lbl} ????!\n"
            f"????: {title}\n"
            f"?????: {count}\n"
            f"?????: {emoji}",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("???? ????", url=f"https://t.me/addstickers/{short}")]]
            )
        )
