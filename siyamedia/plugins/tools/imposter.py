...
from siyamedia.mongo.pretenderdb import (
...
async def set_mataa(_, message: Message):
    if len(message.command) == 1:
        return await message.reply("?????? ????????? ?s??s **?s???:** `/imposter enable|disable`")
    if message.command[1] == "enable":
        ...
        if cekset:
            ...
            await message.reply(f"**s????ss????? ??????? ????????? ???? ???** {message.chat.title}")
    elif message.command[1] == "disable":
        cekset = await impo_off(message.chat.id)
        if not cekset:
            await message.reply("**????????? ???? ?s ??????? ??s?????.**")
        else:
            await impo_off(message.chat.id)
            await message.reply(f"**s????ss????? ??s????? ????????? ???? ???** {message.chat.title}")
    else:
        await message.reply("**?????? ????????? ?s??s ?s??? : ????????? ??|???**")
