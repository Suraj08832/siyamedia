...
async def weather_command(client: Client, message: Message):
    ...
    try:
        ...
        weather_text = (
            f"<b>{location_name}</b> ??\n\n"
            f"??? <b>???????????:</b> <code>{obs.get('temperature', 'N/A')} °C</code>\n"
            f"?? <b>????s ????:</b> <code>{obs.get('temperatureFeelsLike', 'N/A')} °C</code>\n"
            f"?? <b>????????:</b> <code>{obs.get('relativeHumidity', 'N/A')}%</code>\n"
            f"?? <b>????:</b> <code>{obs.get('windSpeed', 'N/A')} km/h</code>\n"
            f"?? <b>?????????:</b> <i>{obs.get('wxPhraseLong', 'N/A')}</i>"
        )
